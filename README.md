...
- **Vault** (`%APPDATA%\com.authenticator.app\vault.json`): key được tạo bằng Argon2id từ
  master password, dữ liệu mã hoá AES-256-GCM. Key chỉ nằm trong RAM khi vault mở và bị
  zeroize khi khoá.
- Protocol extension ↔ app (`packages/core/src/protocol.ts`): `ping`, `status`, `list_codes`, `add_uri`.

## Tính năng

- Tạo vault với master password, mở khoá và khoá lại
...
```bash
cd desktop/src-tauri && cargo test   # vector RFC 6238, parse otpauth, vault, framing native messaging
pnpm typecheck
```

## Hướng phát triển tiếp

- Tự khoá vault sau một khoảng thời gian không dùng; đổi master password
- Import từ Google Authenticator (`otpauth-migration://`, protobuf), export và backup mã hoá
